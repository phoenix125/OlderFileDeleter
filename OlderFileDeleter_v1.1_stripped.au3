...
SplashOff()
Local $tMB = MsgBox($MB_YESNOCANCEL, $aUtilityVer, "Scan and delete files now?" & @CRLF & @CRLF & "Click (YES) to run NOW." & @CRLF & "Click (NO) to skip this instance." & @CRLF & "Click (CANCEL) to exit program.", $aPromptTimeout)
If $tMB = 6 Then
...
LogWrite("[DEL QTY] Deleting " & ($t + 1) & " of " & (UBound($xFilesDeleteQnty) - 2) & ":" & $xFilesDeleteQnty[$t])
...
Global $iTrayRestart = TrayCreateItem("Restart program")
TrayItemSetOnEvent(-1, "TrayRestart")
TrayCreateItem("")
Global $iTrayOpenLatestLog = TrayCreateItem("Open latest log")
TrayItemSetOnEvent(-1, "TrayOpenLatestLog")
Global $iTrayOpenLogFolder = TrayCreateItem("Open log folder")
TrayItemSetOnEvent(-1, "TrayopenLogFolder")
Global $iTrayViewConfig = TrayCreateItem("Open config file")
...
$xArray[9] = 'timeout 3'
$xArray[10] = 'exit'
Local $tBatFile = @ScriptDir & "\" & $aUtilName & "_Delay_Restart.bat"
FileDelete($tBatFile)
...
Exit
EndFunc
Func TrayOpenLatestLog()
Local $aLogFile = $aFolderLog & $aUtilName & "_Log_" & @YEAR & "-" & @MON & "-" & @MDAY & ".txt"
...
EndFunc
Func TrayopenLogFolder()
ShellExecute($aFolderLog)
EndFunc
Func TrayViewConfig()
...
Local $tDelete = 0
For $n = 1 To (UBound($xFileMatch) - 1)
Local $tSplit = StringSplit($xFileMatch[$n], "?")
Local $tFldr = $tSplit[1]
Local $tFN = $tSplit[2]
Local $tAge = $tSplit[3]
Local $tNum = $tSplit[4]
For $m = 0 To (UBound($xFileAge) - 1)
...
EndFunc
Func _ExitUtil()
MsgBox(0, $aUtilityVer, "Thank you for using " & $aUtilName & "." & @CRLF & @CRLF & "Please report any problems or comments to: " & @CRLF & "Discord: http://discord.gg/EU7pzPs or " & @CRLF & "Forum: http://phoenix125.createaforum.com/index.php. " & @CRLF & @CRLF & "Visit http://www.Phoenix125.com", 20)
Exit
EndFunc
Func LogWrite($Msg)
